 ...
 */
int bst_size2(struct bst_node* n) {
	int num = 0;

	if (n->left != NULL) {
		num++;
		num = num + bst_size2(n->left);
	}
	if (n->right != NULL) {
		num++;
		num = num + bst_size2(n->right);
	}
 ...
 */
int bst_height2(struct bst_node *n, int level) {
	int level_right = 0;
	int level_left = 0;

	if (n->right != NULL) {
		level_right = bst_height2(n->right, level + 1);
	}
	if (n->left != NULL) {
		level_left = bst_height2(n->left, level + 1);
	}
	if (level_right > level_left) {
		return level_right;
	}
	else if (level_left > level ) {
		return level_left;
	}
	else {
		return level;
 ...
 */
int bst_path_sum2(struct bst_node* n, int total, int sum, int* contained) {
	if(n->left != NULL) {
		bst_path_sum2(n->left, total + n->val, sum, contained);
	}
	...
	if(n->right != NULL) {
		bst_path_sum2(n->right, total + n->val, sum, contained);
	}
	else if (n->left == NULL && n->right == NULL) {
		if(total + n->val == sum) {
			*contained = 1;
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
	...
	iter->parent = bst->root;
	iter->stack1 = stack_create();
	iter->stack2 = stack_create();
	stack_push(iter->stack2, bst->root);
	return iter;
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
	if(stack_isempty(iter->stack1) && stack_isempty(iter->stack2)) {
		return 0;
	}
...
void bst_iterator_next2(struct bst_node* n, struct bst_iterator* iter) {
	if (n != NULL) {
		bst_iterator_next2(n->left, iter);
		stack_push(iter->stack2, n);
		bst_iterator_next2(n->right, iter);
	}
}
 ...
 */
/*int bst_iterator_next2(struct bst_node* n, int* counter) {
	 if(n->left != NULL) {

		 return bst_iterator_next2(n->left, &counter);
	 }
	 if(n->right != NULL) {

		 return bst_iterator_next2(n->right, &counter);
	 }
...
int bst_iterator_next(struct bst_iterator* iter) {
	struct bst_node* s;
	if(stack_isempty(iter->stack1) && !stack_isempty(iter->stack2)) {
		bst_iterator_next2(iter->node, iter);
